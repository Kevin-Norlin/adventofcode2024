...
def is_possible(result, values):
    operators = generate_operators_recursively(len(values)-1)
...
def apply_op(op: str, v1 : int, v2: int) -> int:
    ...
    if op == '+':
        return v1 + v2
    # Case || --> Combine 15 and 6 to 156
    return int(str(v1) + str(v2))
    
def generate_operators_recursively(n : int):
    return gen_op_rec(n, list([] for _ in range(pow(len(OPERATORS),n))))
...
def gen_op_rec(n: int, l: list[list[str]]):
    ...
    for i in range(length):
        l[i].append(OPERATORS[op % len(OPERATORS)])
        if i % (pow(len(OPERATORS),n) // len(OPERATORS)) == 0:
            op += 1
